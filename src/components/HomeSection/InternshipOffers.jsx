...
export default function InternshipOffers() {
  ...
  return (
    <section className="py-10 bg-gradient-to-br from-gray-50 to-blue-50 relative overflow-hidden ">
      <div className="absolute top-10 left-0 w-72 h-72 bg-blue-200 rounded-full mix-blend-multiply filter blur-xl opacity-20 animate-blob"></div>
      <div className="absolute top-0 right-0 w-72 h-72 bg-purple-200 rounded-full mix-blend-multiply filter blur-xl opacity-20 animate-blob animation-delay-2000"></div>
      <div className="absolute bottom-0 left-1/2 w-72 h-72 bg-amber-200 rounded-full mix-blend-multiply filter blur-xl opacity-20 animate-blob animation-delay-4000"></div>

      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          <div className="inline-flex items-center px-3 py-1 rounded-full bg-gradient-to-r from-green-100 to-orange-100 text-gray-700 text-sm font-semibold mb-4 shadow-sm">
            <Zap className="w-4 h-4 mr-2 text-green-500" />
            Limited Spots Available - Apply Before They're Gone!
          </div>
          <h1 className="text-3xl md:text-5xl text-gray-900 my-3">
            Launch Your <span className="text-transparent font-semibold bg-clip-text bg-gradient-to-r from-blue-600 to-purple-600">Tech Career</span>
          </h1>
          <p className="text-gray-600 text-lg max-w-2xl mx-auto mb-8">
            Join thousands of students who transformed their careers through our industry-focused internship programs
          </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10 mb-12">
          {filteredInternships.map((internship, index) => (
            <div
              ...
              ref={el => cardRefs.current[index] = el}
              data-id={internship.id || internship._id}
              className="group bg-white rounded-md border border-gray-200 shadow-sm hover:shadow-2xl transition-all duration-500 overflow-hidden hover:border-blue-300 relative"
            >
              {internship.isFeatured && (
                <div className="absolute top-4 left-4 z-10">
                  <div className="bg-gradient-to-r from-amber-400 to-orange-500 text-white text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1 shadow-lg">
              ...
              <div className="p-4">
                ...
                <div className="flex justify-between items-center pt-2 border-t border-gray-100">
                  <button
                    onClick={() => handleApplyNow(internship.domain)}
                    className="flex-1 mr-3 bg-gradient-to-r from-sky-400 to-sky-600 text-white py-1.5 rounded-xl font-semibold hover:from-sky-600 hover:to-sky-400  hover:shadow-xl group/apply flex items-center justify-center gap-2"
                  >
                    <Target className="w-4 h-4" />
                    Apply Now
                    <ArrowRight className="w-4 h-4 group-hover/apply:translate-x-1 transition-transform" />
                  ...
                  <div className="">
                    <Link
                      to={`/internship/${String(internship.domain || "").toLowerCase().replace(/\s+/g, "-")}`}
                      className="flex items-center p-1 px-5 text-gray-600 hover:text-blue-600 transition-all duration-300 hover:underline group/knowmore"
                      title="View Details"
                    >
                      <Play className="w-4 h-4 mr-2 group-hover/knowmore:scale-110 transition-transform" /> Know More
                    </Link>
